...
MODULE grid
!*******************************************************************************
! Generates all the shared grid data, with functions at the bottom (none of which are called other than in this script - I think)
!*******************************************************************************
    USE shared_data
    USE gridpoint_tools
    ...
    subroutine establish_grid()
        ...
        !The initial points have been found in the main process. These now need to be rotated and sent to the other processes. May as well just keep as one large array here rather than being clever with MPI
        call find_all_points()   !Finds the location of the raw points over the whole sphere

        call populate_global_grids()  !Determines information on geometry and grid neighbours etc.

        call extend_to_3d()  !Extends all this to 3d

        call calculate_grid_extras()  !Calculates the matrices necessary for the gridpoint averaging
    ...
    subroutine calculate_grid_extras()
        ...
        integer:: i, k, ri, i1, i2, j
        ! Calculate the cell normals, tangents, coefficients etc.
        ...
        allocate(uvs(0:1,0:ntri+nghosttri-1,seg_min:seg_max))
        ...
        allocate(pts_extend(0:2,0:nr+3,0:npts+nghosts-1,0:19))


        do ri = 0, nr+3
        trisides_extend(0:2,ri,:,:,:) = trisides(:,:,:,:)
        centres_extend(0:2,ri,:,:) = centres(:,:,:)
        pts_extend(0:2,ri,:,:) = all_base_pts(:,:,:)

        end do

        allocate(bvec(0:2,0:11,0:npts-1,seg_min:seg_max))
        allocate(bmat(0:2,0:2,0:npts-1,seg_min:seg_max))

        allocate(jvec(0:2,0:6,0:npts-1,seg_min:seg_max))
        allocate(jmat(0:2,0:2,0:npts-1,seg_min:seg_max))
        ...
        bvec(:,j,k,seg) = trinorms(:,trimap(1,j,k),trimap(0,j,k),seg)
        jvec(:,j,k,seg) = trisides(:,trimap(1,j,k),trimap(0,j,k),seg)
        end do

        do j = 0, nnbrs(k)-1
        bvec(:,j+nnbrs(k),k,seg) = centres(:,trimap(0,j,k),seg)
        end do
        jvec(:,nnbrs(k),k,seg) = all_base_pts(:,k,seg)

        do i1 = 0,2  !Components of the matrix obtained with the ATA of a_vec^T
        do i2 = 0,2
        bmat(i1,i2,k,seg) = sum(bvec(i1,0:2*nnbrs(k)-1,k,seg)*bvec(i2,0:2*nnbrs(k)-1,k,seg))
        jmat(i1,i2,k,seg) = sum(jvec(i1,0:nnbrs(k),k,seg)*jvec(i2,0:nnbrs(k),k,seg))
    ...
    subroutine extend_to_3d()   !Need to take care here with regard to the splitting in the radial direction
        implicit none
        integer:: ri
        ...
        allocate(all_ds(0:nr +1,0:2,0:ntri+nghosttri-1,0:19))
        allocate(all_point_ds(0:nr +1,0:5,0:npts-1,0:19))
        allocate(tri_d_all(0:nr +1,0:2,0:ntri+nghosttri-1,0:19))
        ...
        allocate(dual_volumes(1:nr +1,0:npts-1,0:19))

        allocate(v_duals(1:nr +1,0:2,0:ntri+nghosttri-1,0:19))

        allocate(h_areas_global(0:nr_global +2,0:ntri+nghosttri-1,0:19))
        allocate(v_areas_global(0:nr_global +1,0:2,0:ntri+nghosttri-1,0:19))
        ...
        do ri = 1, nr +1
            !Don't have to do the exception to the rule!! Careful to pass this correctly though, numbering starts at 1. Vertical cell areas on the dual grid.
            v_duals(ri,:,:,:) = 0.5_d*(rc_global(ri + seg_layer*nr)**2 - rc_global(ri + seg_layer*nr - 1)**2)*tri_d(:,:,:)
            dual_volumes(ri,:,:)=(1.0_d/3.0_d)*(rc_global(ri + seg_layer*nr)**3-rc_global(ri + seg_layer*nr-1)**3)*dual_areas(:,:)
        end do

        do ri = 0, nr_global+2
            h_areas_global(ri,:,:) = rs_global(ri)**2*areas(:,:)
        end do

        do ri = 0, nr_global+1
            v_areas_global(ri,:,:,:) = 0.5_d*(rs_global(ri+1)**2 - rs_global(ri)**2)*tri_l(:,:,:)
        end do
    ...
    subroutine populate_global_grids()
        ...
        integer:: i, i1, j, k, kt, tk, u, v, g1, gi
        ...
        rs_global = rsun*exp(ps)
        rc_global = rsun*exp(pc)
        ...
        !Construct duel grid faces as arrays of hexagons/pentagons
        ...
        nbrpts(:,0) = [ n+1, 1, npts+n-1, npts+n, npts+n+1, 0]

        nbrpts(:,npts-1) = [npts-2, npts-3, npts+2*n, npts+2*n+1, npts+2*n+2, 0]
        nbrpts(:,n) = [n-1,  2*n, npts+3*n+1, npts+3*n+2,npts, 0]
        ...
        !New trimap bit for the new averaging
        tri_c = 0
    ...
    !That's all the integer stuff. Can now calculate the locations of points etc.
    ...
    !By running though tri_transfer we can first do the cirumcentres, then the points around them. Providing everything at least matches up a bit.
    !That way even the edges should match exactly, which would be nice

    do seg = 0, 19
        do gi = 0, nghosttri - 1
        centres(:,ntri+gi,seg) = centres(:,tri_transfer(1,gi,seg),tri_transfer(0,gi,seg))
        do i = 0, 2
            if (triangs(i,ntri+gi) < npts) then  !On an edge, do all these first in case they're used in the ghost points, wchih happens at the corners
                pt0 = all_base_pts(:,triangs(i,ntri+gi),seg)
                pt1 = all_base_pts(:,triangs(mod(i+tri_transfer(2,gi,seg), 3),tri_transfer(1,gi,seg)),tri_transfer(0,gi,seg))
                all_base_pts(:,triangs(i,ntri+gi),seg) = pt0
                all_base_pts(:,triangs(mod(i+tri_transfer(2,gi,seg), 3),tri_transfer(1,gi,seg)),tri_transfer(0,gi,seg)) = &
                pt0
            end if
        end do
        end do
    ...
    do seg = 0, 19
        do  gi = 0, nghosttri - 1
        do i = 0, 2
        if (triangs(i,ntri+gi) .ge. npts) then  !Is a ghost point, take directly from the interior (or feasibly, the edge)
            all_base_pts(:,triangs(i,ntri+gi),seg) = &
            all_base_pts(:,triangs(mod(i+tri_transfer(2,gi,seg), 3),tri_transfer(1,gi,seg)),tri_transfer(0,gi,seg))
        end if
        end do
        end do
    ...
    allocate(tri_d(0:2,0:ntri+nghosttri-1,0:19))
    ...
    !A new modification here = need to transfer tri_d information between segments, as we're now calculating extended b from a
    do seg = 0, 19
        do gi = 0, nghosttri - 1
        tri_d(:,ntri+gi,seg) = tri_d(:,tri_transfer(1,gi,seg),tri_transfer(0,gi,seg))
        end do
    end do
