...
class Subdomain:
    ...
    def __init__(self, triang, nbrs, corners, n, pts, rmax, seg):
        ...
        # Record identity of neighbouring grid faces[not ghost faces]:
        # ---------------------------------------------------------------
        # these are ordered anticlockwise but I'm not sure yet how to specify more than that
        # must take care to define distances and things correctly as a result
        # more complex than calculating for points as some are upward, some downward
        self.nbrfaces = np.zeros((self.ntri, 3), dtype = 'int16')
        self.updown = np.zeros((self.ntri), dtype = 'int16')  #whether the triangle is up or down
        # Find triangle indices given u,v point.
        #The two trianlges are to the LEFT of the point, with p0 at the top.
        def findt(u,v,n,up):
            if up:
        ...
        for u in range(n):
            for v in range(u + 1, n + 1):   
                ...
                if v < n:
                    n3 = findt(u, v, n, 0)
                if u > 0:
                    n5 = findt(u-1, v-1, n, 0)
                self.nbrfaces[tk, :] = np.array([n1,n3,n5])
                self.updown[tk] = 1
                if v < n:   #there is both of them
                    tk = findt(u, v, n, 0)   #downward triangle
                    n0, n2, n4 = -1, -1, -1
                    n0 = findt(u, v, n, 1)
                    n2 = findt(u+1, v+1, n, 1)
                    n4 = findt(u, v+1, n, 1)
                    self.nbrfaces[tk, :] = np.array([n0,n2,n4])
                    self.updown[tk] = 0
        # Populate ghost triangles
        for i, ghostk in enumerate(range(n**2, n**2 + 2*n -1, 2)):
            self.nbrfaces[findt(0, n-i, n, 1), 2] = ghostk
        for i, ghostk in enumerate(range(n**2 + 2*n + 1, n**2 + 4*n, 2)):

            self.nbrfaces[findt(i, i+1, n, 1), 0] = ghostk
        for i, ghostk in enumerate(range(n**2 + 4*n + 2, n**2 + 6*n + 1, 2)):

            self.nbrfaces[findt(n-i-1, n, n, 1), 1] = ghostk


        # Distances from neighbouring (raw) grid points
        # ---------------------------------------------
        self.nbr_dist = np.zeros((npts, 6))
        for k, pt in enumerate(self.pts[:npts]):  # omit ghost pts
            for j in range(self.nnbrs[k]):
                self.nbr_dist[k,j] = self.distance(pt, self.pts[self.nbrpts[k,j]])
                
                
        # Lengths of dual edges around each (raw) grid point
        # --------------------------------------------------
        self.nbr_lens = np.zeros((npts, 6))
        for k, pt in enumerate(self.pts[:npts]):  # omit ghost points
            for j in range(self.nnbrs[k]):
                cen0 = self.centres[self.faces[k, j]]
                cen1 = self.centres[self.faces[k, (j+1)%6]]
                self.nbr_lens[k,j] = self.distance(cen0, cen1)
        # Compute areas of raw grid cells (triangles)
        # -------------------------------------------
        self.areas = np.zeros(ntri + nghosttri)
        for kt, triang in enumerate(self.triangs):
            self.areas[kt] = self.triangle_area(self.pts[triang[0]], self.pts[triang[1]], self.pts[triang[2]])
            
        # Compute areas of dual grid cells (hexagons or pentagons)
        # --------------------------------------------------------
        self.dual_areas = np.zeros(npts)
        for k in range(npts):  # omit ghost points
            for j in range(self.nnbrs[k]):  # split into triangles
                cen1 = self.centres[self.faces[k, j]]
                cen2 = self.centres[self.faces[k, (j+1)%6]]
                self.dual_areas[k] += self.triangle_area(self.pts[k], cen1, cen2)


        #Distance information for the triangles.
        self.tri_d = 1e-10*np.ones((self.ntri + self.nghosttri, 3))
        self.tri_l = 1e-10*np.ones((self.ntri + self.nghosttri, 3))
        for kt in range(ntri):
            for j in range(3):
                self.tri_d[kt,j] = self.distance(self.centres[kt], self.centres[self.nbrfaces[kt,j]])
        for kt in range(ntri + nghosttri):
            for j in range(3):
                self.tri_l[kt,j] = self.distance(self.pts[self.triangs[kt, j]], self.pts[self.triangs[kt, (j+1)%3]])


        def side_tri(side, i, n):  #returns index of triangles along a certain side.
            #Ordered CLOCKWISE to match up with the other segment, where it will go in the other direction.
            if side == 0:
                i2 = 2*n - i - 2
                return 2*n*(i2//2) - (i2//2)**2 + i2%2
            if side == 1:
                return (2*n-2)*((i//2)+1) - (i//2)**2 - i%2
            if side  == 2:
                return i

        #tri_transfer maps a ghost triangle to the real triangle in the adjacent segment (or not adjacent if it's on a corner)
        #Now also contains orientation data for each triangle
        self.tri_transfer = np.zeros((len(ghst_tri), 3),dtype = 'int16')
        
        #print(self.edge_transfer)
        #Corner ghost faces. There is some redundancy here but it seems to work out.
        
        corner_faces = np.array([0, n**2 - 1, 2*(n-1)])  #indices of the faces at each corner (anticlockwise)
        for i, g in enumerate(range(n**2 + 2*n -1, n**2 + 2*n + 1)):   #corner 0
            self.tri_transfer[g - self.ntri] = np.array([self.corners[seg,0,i+1,0], corner_faces[self.corners[seg,0,i+1,1]],self.corners[seg,0,i+1,1]])
        for i, g in enumerate(range(n**2 + 4*n, n**2 + 4*n + 2)):   #corner 1
            self.tri_transfer[g - self.ntri] = np.array([self.corners[seg,1,i+1,0], corner_faces[self.corners[seg,1,i+1,1]],self.corners[seg,1,i+1,1]])
        for i, g in enumerate(range(n**2 + 6*n + 1, n**2 + 6*n + 3)):   #corner 2
            self.tri_transfer[g - self.ntri] = np.array([self.corners[seg,2,i+1,0], corner_faces[self.corners[seg,2,i+1,1]],self.corners[seg,2,i+1,1]])
        

        #Side ghost faces
        for i, g in enumerate(range(n**2 + 2*n + 1, n**2 + 4*n)):   #side 0 

            if self.nbrs[seg][0,1] == 0:   #side that this maps to
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][0,0], side_tri(self.nbrs[seg][0,1], i, n),-1]   #this shouldn't happen within the established setup of side combinations
            if self.nbrs[seg][0,1] == 1:
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][0,0], side_tri(self.nbrs[seg][0,1], i, n),2*(g%2)]
            if self.nbrs[seg][0,1] == 2:
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][0,0], side_tri(self.nbrs[seg][0,1], i, n),((g-1)%2)]
        for i, g in enumerate(range(n**2 + 4*n + 2, n**2 + 6*n + 1)):   #side 1 

            if self.nbrs[seg][1,1] == 0:   #side that this maps to
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][1,0], side_tri(self.nbrs[seg][1,1], i, n),g%2]
            if self.nbrs[seg][1,1] == 1:
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][1,0], side_tri(self.nbrs[seg][1,1], i, n),-1]  #also shouldn't happen
            if self.nbrs[seg][1,1] == 2:
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][1,0], side_tri(self.nbrs[seg][1,1], i, n),2*((g-1)%2)]
        for i, g in enumerate(range(n**2, n**2 + 2*n - 1)):   #side 2 
            if self.nbrs[seg][2,1] == 0:   #side that this maps to
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][2,0], side_tri(self.nbrs[seg][2,1], i, n),2*((g-1)%2)]
            if self.nbrs[seg][2,1] == 1:
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][2,0], side_tri(self.nbrs[seg][2,1], i, n),g%2]
            if self.nbrs[seg][2,1] == 2:
                self.tri_transfer[g - self.ntri] = [self.nbrs[seg][2,0], side_tri(self.nbrs[seg][2,1], i, n),-1]   #shouldn't happen
        

        'The following is just for extending into 3d'
        #Extend various things into 3D, including the point locations and areas of faces etc. 
        self.all_pts = np.array([self.pts*self.rs[i] for i in range(len(self.rs))])
        
        self.all_centres = np.array([self.centres*self.rc[i] for i in range(len(self.rs)-1)])
        self.all_ds = np.array([self.tri_d*self.rc[i] for i in range(len(self.rs)-1)])
        self.h_areas = np.array([self.areas*self.rs[i]**2 for i in range(len(self.rs))] )

        self.v_areas = 0.5*np.array([self.tri_l*(self.rs[i+1]**2 - self.rs[i]**2) for i in range(len(self.rs)-1)] )
        self.tri_l_mids = np.array([self.tri_l*(self.rc[i]) for i in range(len(self.rs)-1)])
        self.tri_l_faces = np.array([self.tri_l*self.rs[i] for i in range(len(self.rs))])
        self.tri_d_all = np.array([self.tri_d*(self.rc[i]) for i in range(len(self.rs)-1)] )
        self.cell_volumes = np.array([self.areas*(self.rs[i+1]**3 - self.rs[i]**3)/3 for i in range(len(self.rs) - 1)])
        self.all_duals = np.array([self.dual_areas*self.rc[i]**2 for i in range(len(self.rs)-1)])

        self.v_duals = 0.5*np.array([self.tri_d*(self.rc[i+1]**2 - self.rc[i]**2) for i in range(0,self.nr+1)])   #EXCEPTION TO THE RULE - numbering-wise
